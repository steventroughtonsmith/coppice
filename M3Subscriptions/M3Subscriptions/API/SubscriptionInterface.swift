...
protocol SubscriptionAPI {
    func activate(_ request: ActivationRequest, device: Device, completion: (Result<SubscriptionInfo, ActivateAPI.Failure>) -> Void)
    func check(_ device: Device, token: String, completion: (Result<SubscriptionInfo, CheckAPI.Failure>) -> Void)
    func deactivate(_ device: Device, token: String, completion: (Result<SubscriptionInfo, DeactivateAPI.Failure>) -> Void)
}
