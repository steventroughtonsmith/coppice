...
/*class ArrowLayoutEngineTests: XCTestCase {
    ...
    func test_calculateArrows_childDirectlyToLeftOfParent() throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1])
    ...
    func test_calculateArrows_childrenAtEachSideOfParent() throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, left, top, right, bottom])

        let arrows = engine.calculateArrows()
        XCTAssertEqual(arrows.count, 4)
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 0, y: 20, edge:  .left, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: -12, y: 9, edge:  .right, page: left))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 10, y: 0, edge:  .top, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 9, y: -12, edge:  .bottom, page: top))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 20, y: 20, edge:  .right, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 32, y: 9, edge:  .left, page: right))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 10, y: 40, edge:  .bottom, page: self.rootPage),
    ...
    func test_calculateArrows_childToTopLeftOfParent() throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child])
        let arrows = engine.calculateArrows()
        XCTAssertEqual(arrows.count, 1)
    ...
    func test_calculateArrows_childrenAtEachCornerOfParent() {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, leftTop, topRight, rightBottom, bottomLeft])
    ...
    func test_calculateArrows_multipleChildrenToLeftOfParent() {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1, child2, child3])

        let arrows = engine.calculateArrows()
        XCTAssertEqual(arrows.count, 3)

        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 0, y: 7, edge:  .left, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: -10, y: -13, edge:  .right, page: child1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 0, y: 20, edge:  .left, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: -30, y: 5, edge:  .right, page: child2))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 0, y: 33, edge:  .left, page: self.rootPage),
    ...
    func test_calculateArrows_multipleChildrenAboveParent() {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1, child2])

        let arrows = engine.calculateArrows()
        XCTAssertEqual(arrows.count, 2)

        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 5, y: 0, edge:  .top, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 0, y: -10, edge:  .bottom, page: child1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 15, y: 0, edge:  .top, page: self.rootPage),
    ...
    func test_calculateArrows_multipleChildrenToRightOfParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 20, y: 10, edge:  .right, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 30, y: -13, edge:  .left, page: child1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 20, y: 30, edge:  .right, page: self.rootPage),
    ...
    func test_calculateArrows_multipleChildrenBelowParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 3, y: 40, edge:  .bottom, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 0, y: 50, edge:  .top, page: child1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 17, y: 40, edge:  .bottom, page: self.rootPage),
    ...
    func test_calculateArrows_multipleLevelsOfChildrenToLeftOfParent() throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1, grandChild1])
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: -30, y: 10, edge:  .left, page: child1),
    ...
    func test_calculateArrows_multipleLevelsOfChildrenAboveParent()  throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1, grandChild1, grandChild2])
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 8, y: -15, edge:  .top, page: child1),
                                                        endPoint: self.arrowPoint(x: 10, y: -30, edge:  .bottom, page: grandChild1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 13, y: -15, edge:  .top, page: child1),
    ...
    func test_calculateArrows_multipleLevelsOfChildrenToRightOfParent()  throws {
        ...
        let engine = LinkLayoutEngine(pages: [self.rootPage, child1, grandChild1, greatGrandChild1])
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 50, y: 10, edge:  .right, page: child1),
                                                        endPoint: self.arrowPoint(x: 80, y: 15, edge:  .left, page: grandChild1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 90, y: 15, edge:  .right, page: grandChild1),
    ...
    func test_calculateArrows_multipleLevelsOfChildrenBelowParent()  throws {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 10, y: 40, edge:  .bottom, page: self.rootPage),
                                                        endPoint: self.arrowPoint(x: 0, y: 50, edge:  .top, page: child1))))
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 0, y: 60, edge:  .bottom, page: child1),
    ...
    func test_calculateArrows_childToLeftOfGrandChildAndParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: -90, y: 13, edge:  .right, page: child1),
    ...
    func test_calculateArrows_childAboveGrandchildAndParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 18, y: -90, edge:  .bottom, page: child1),
    ...
    func test_calculateArrows_childToRightOfGrandchildAndParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 100, y: 18, edge:  .left, page: child1),
    ...
    func test_calculateArrows_childBelowGrandchildAndParent() {
        ...
        XCTAssertTrue(arrows.contains(LayoutEngineLink(startPoint: self.arrowPoint(x: 23, y: 100, edge:  .top, page: child1),
