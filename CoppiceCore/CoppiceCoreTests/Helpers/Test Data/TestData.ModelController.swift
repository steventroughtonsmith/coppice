        ...
        lazy var canvasLinks: [CanvasLink] = {
            return [
                self.modelController.canvasLinkCollection.newObject() {
                    ...
                    $0.link = PageLink(destination: Page.modelID(with: self.pageIDs[1]), source: Page.modelID(with: self.pageIDs[0]), autoGenerated: false)
                    $0.destinationPage = self.canvasPages[1]
                    $0.sourcePage = self.canvasPages[0]
                },
                self.modelController.canvasLinkCollection.newObject() {
                    $0.id = CanvasLink.modelID(with: self.canvasLinkIDs[1])
                    $0.link = PageLink(destination: Page.modelID(with: self.pageIDs[2]), source: Page.modelID(with: self.pageIDs[1]), autoGenerated: false)
