...
extension Page.Content {
    public class Image: Page.Content {
        public override var contentType: Page.ContentType {
            return Page.ContentType.image
        }

        @Published public private(set) var image: NSImage? {
            didSet {
                self.didChange(\Image.image, oldValue: oldValue)
            }
        }
        ...
        @Published public var imageDescription: String? {
            didSet {
                ...
                self.didChange(\Image.imageDescription, oldValue: oldValue)
            }
        }
        ...
        @Published public var cropRect: CGRect {
            didSet {
                ...
                self.undoManager?.beginUndoGrouping()
                self.didChange(\Image.cropRect, oldValue: oldValue)
        ...
        @Published public var hotspots: [ImageHotspot] = [] {
            didSet {
                ...
                self.findAllLinks()
                self.didChange(\Image.hotspots, oldValue: oldValue)
            }
        }
        ...
        @Published public var recognizedTexts: [VNRecognizedText] = [] {
            didSet {
                ...
                self.didChange(\Image.recognizedTexts, oldValue: oldValue)
            }
        }
        ...
        public init(modelFile: ModelFile) throws {
            ...
            if let metadata = modelFile.metadata {
                ...
                otherMetadata = metadata.filter { (key, _) in
...
extension CGImagePropertyOrientation {
    func rotated(by direction: Page.Content.Image.ImageOperation.RotationDirection) -> CGImagePropertyOrientation {
