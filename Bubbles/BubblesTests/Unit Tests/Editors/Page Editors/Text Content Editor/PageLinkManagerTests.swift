...
class PageLinkManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        self.editingPage = self.modelController.collection(for: Page.self).newObject() { $0.title = "Hello World" }
    ...
    func test_updatesLinksWhenTextStorageFirstSet() {
        ...
        XCTAssertEqual(url, self.editingPage.linkToPage(autoGenerated: true).url)
    }
    ...
    func test_updatesLinksWhenAPageIsRemoved() {
        ...
        self.performAndWaitFor("Did Finish Parsing Links") { (expectation) in
            delegate.expectation = expectation
            self.modelController.collection(for: Page.self).delete(self.editingPage)
        }
    ...
    func test_updatesLinksWhenAPageTitleChanges() {
        ...
        self.performAndWaitFor("Did Finish Parsing Links") { (expectation) in
            delegate.expectation = expectation
            self.editingPage.title = "test"
        }

        XCTAssertNil(storage.attribute(.link, at: 5, effectiveRange: nil) as? URL)
        let expectedURL = self.editingPage.linkToPage(autoGenerated: true).url
